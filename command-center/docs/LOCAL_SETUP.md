...
2. **Obsidian (writes + sync API)** still needs a vault path and write flag (see [Obsidian Sync](#obsidian-sync) below). **Ruflo Agents** and **Hermes:** the web UI **always** calls `/api/ruflo/feed` and `/api/hermes/feed` on inbox refresh. Enable the matching `JARVIS_ADAPTER_*` flags and emit audit rows tagged `ruflo` / `swarm` / `claude_flow` or `hermes` / `dispatcher` / `handoff` so those channels show live rows instead of only fallback cards. GoodMood uses the same audit pattern with `good_mood`, `coach`, `mood`.
